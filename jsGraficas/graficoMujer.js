document.addEventListener('DOMContentLoaded', function() {
    const canvas = document.getElementById('graficoProductosMujer'); // Cambiar el ID del canvas
    const ctx = canvas.getContext('2d');
    const xhr = new XMLHttpRequest();
    xhr.open('GET', 'graficaPruebas/getDataMujer.php', true); // Cambiar la ruta del archivo PHP
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            productosMasVendidos.forEach((producto, i) => {
                ...
                ctx.fillText(producto.nombre, x + barraAncho / 2 - 10, canvas.height);
